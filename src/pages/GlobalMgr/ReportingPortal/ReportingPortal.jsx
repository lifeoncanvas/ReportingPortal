...
function ClarificationModal({ report, endpoint, onClose, onDone }) {
  ...
  const baseUrl = window.ENV?.API_PATH || process.env.REACT_APP_API_URL || 'https://65.2.153.58:8080';
...
function ZonalReportForm({ onClose, onSubmit: parentSubmit }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await fetch(`${window.ENV?.API_PATH || process.env.REACT_APP_API_URL || 'https://65.2.153.58:8080'}/api/reports`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSubmit = async (data) => {
    try {
      if (['partnership', 'testimonials', 'magazine', 'outreach'].includes(activeTab)) {
        const res = await fetch(`${window.ENV?.API_PATH || process.env.REACT_APP_API_URL || 'https://65.2.153.58:8080'}/api/portal-reports/${activeTab}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            ...data,
